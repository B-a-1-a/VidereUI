...
### Preset E — "Ethereal Flow" (Pastel Canvas)
- **Identity:** A serene, cloud-like digital presentation. Soft, expansive, and driven by massive, confident typography rather than heavy UI.
- **Palette:** Icy Cyan `#DCEFFA` to Soft Blue `#C1E3F5` gradient (Background), Deep Indigo `#111827` (Text/Dark), Pure White `#FFFFFF` (Surface), Soft Accent `#93C5FD` (Interactive).
- **Typography:** Headings: "Cabinet Grotesk" or "Clash Display" (Massive, uppercase, ultra-tight tracking). Drama: "Instrument Serif" Italic. Data: `"Space Mono"`.
- **Image Mood:** Frosted glass, light leaks, soft sky gradients, translucent acrylic, minimalist pale blue textures.
- **Hero line pattern:** "[Visionary word]" (Massive Sans, filling the screen) / "[Gentle verb]." (Elegant Serif Italic)

---

## Layout Override: "The Horizontal Deck"
**CRITICAL RULE:** If Preset E is selected (or if the user explicitly requests the horizontal deck), **abandon the vertical scroll structure (Sections D, E, F, G) completely.** The application becomes a fixed `100dvh` full-screen carousel with hidden overflow. 

**Navigation Mechanics:**
- No vertical scrolling. 
- Implement floating, oversized Left (`<`) and Right (`>`) arrow buttons fixed to the sides of the viewport.
- Clicking the arrows transitions the active card using GSAP (`xPercent: -100`, `power3.inOut`, `duration: 1.2`). 

**The 3-Card Architecture:**
1. **Card 1 — "The Title Slide" (Adapts Section B):**
   - Full-bleed pastel gradient background.
   - Website name pinned to the top left.
   - Hero typography is massive, dominating the center of the screen.
   - Minimalist "Begin" or "Next" indicator pointing right.

2. **Card 2 — "The Engine Room" (Adapts Section C):**
   - A soft white glassmorphic container (`bg-white/40 backdrop-blur-2xl`).
   - Contains the 3 "Interactive Functional Artifacts" (Shuffler, Typewriter, Scheduler) laid out cleanly side-by-side or in a refined grid. 
   - Feature rundown text sits above or below the artifacts.

3. **Card 3 — "The Terminal Point" (Adapts the CTA):**
   - The pastel gradient transitions to a slightly deeper, warmer tone.
   - Focuses entirely on a singular, massive Call to Action.
   - Features a giant, magnetic button in the center of the screen (e.g., `scale-150` compared to normal buttons) with a sweeping color-fill hover effect.
   - Minimal footer links docked at the very bottom.
